    ...
    def gen_menu(key, itms):
        ...
        for itm in itms:
            if isinstance(itm, tuple):
                ...
                if model:
                    children.append(format_html('''<dd><a href="javascript:;" href-url="{}">{}</a></dd>''',
                                                model['admin_url'], model['name']))
        return '''<li class="layui-nav-item"><a href="javascript:;">{}</a>
        <dl class="layui-nav-child">{}</dl></li>'''.format(format_html('{}', key), ''.join(children))

    menu_list = [gen_menu(key, itms) for key, itms in settings.SIDE_MENU]
    if 'auth.Group' in modmap:
        menu_list.append(gen_menu('分组', ('auth.Group',)))

    return format_html(''.join(menu_list))
